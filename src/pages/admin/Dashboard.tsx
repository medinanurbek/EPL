import { Shield, Users, Calendar, Trophy, Settings, Activity, ArrowRight, PlusCircle, MessageSquare } from "lucide-react";
                    ...
                    {stats.map((stat) => (
                        <div key={stat.label} className="glass-card p-8 border-l-8 border-l-[#00ff85] shadow-xl rounded-[2rem]">
                            <div className="flex items-center justify-between mb-6">
                                <stat.icon className={`w-8 h-8 ${stat.color === 'text-[#37003c]' ? 'text-white' : stat.color}`} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                    {quickActions.map((action) => (
                        <Link
                            key={action.title}
                            to={action.href}
                            className={`group glass-card p-10 transition-all rounded-[3rem] border border-white/5 shadow-2xl ${action.color}`}
                        >
                            <div className="flex items-start justify-between mb-10">
                                <div className="w-16 h-16 bg-white/5 rounded-2xl flex items-center justify-center text-[#00ff85] group-hover:bg-[#00ff85] group-hover:text-[#37003c] transition-all">
                                    <action.icon className="w-8 h-8" />
                                </div>
                                <PlusCircle className="w-6 h-6 text-white/10 group-hover:text-[#ff2882] transition-colors" />
                            </div>
                            <h3 className="text-2xl font-black text-white mb-4 uppercase tracking-tight">{action.title}</h3>
                            <p className="text-sm text-white/50 font-medium leading-relaxed mb-8 italic">
                                {action.description}
                            </p>
