        ...
        <div className="py-12 bg-slate-50 min-h-screen">
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <header className="mb-16">
                    ...
                    <p className="text-slate-500 font-medium italic">Comprehensive leaderboards for players and clubs.</p>
                </header>

                {/* Player Stats Section */}
                <section className="mb-24">
                    <div className="flex items-center justify-between mb-8">
                        <h2 className="text-2xl font-black text-[#37003c] uppercase tracking-tight">Player Leaders</h2>
                ...
                <section>
                    <div className="flex items-center justify-between mb-8">
                        <h2 className="text-2xl font-black text-[#37003c] uppercase tracking-tight">Club Leaders</h2>
                        <div className="h-px flex-1 mx-8 bg-slate-200 hidden md:block" />
                    </div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex items-center gap-2">
                                <div className="relative w-4 h-4 opacity-50">
                                    <Image src={getTeamLogo(item.team)} alt="logo" fill className="object-contain" />
                                </div>
...
function ClubStatsList({ title, data }: { title: string, data: any[] }) {
    return (
        <div className="glass-card bg-white rounded-[2rem] p-6 shadow-xl border-t-8 border-t-[#37003c] group hover:border-t-[#00ff85] transition-all duration-500">
            <div className="flex items-center justify-between mb-8">
                <h3 className="text-sm font-black text-[#37003c] uppercase tracking-tight">{title}</h3>
                <ChevronRight className="w-4 h-4 text-slate-200 group-hover:text-[#00ff85] transition-colors" />
            </div>
            <div className="space-y-4">
                {data.map((item, i) => (
                    <div key={i} className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <span className="text-sm font-black text-slate-100 w-4">{item.rank}</span>
                            <div className="flex items-center gap-2">
                                <div className="relative w-6 h-6">
                                    <Image src={getTeamLogo(item.team)} alt="logo" fill className="object-contain" />
                                </div>
                                <p className="text-[10px] font-black text-[#37003c] uppercase truncate w-24 sm:w-32">{item.team}</p>
                            </div>
                        </div>
                        <div className="text-sm font-black text-[#37003c] italic">{item.value.toLocaleString()}</div>
                    </div>
                ))}
            </div>
        </div>
